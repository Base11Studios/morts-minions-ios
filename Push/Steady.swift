...
class Steady : Enemy {
    ...
    required init(scalar : Double, defaultYPosition: CGFloat, defaultXPosition: CGFloat, parent: SKNode, value1: Double, value2: Double, scene: GameScene) {
        super.init(scalar: scalar, imageName: "steady_000", textureAtlas: GameTextures.sharedInstance.airAtlas, defaultYPosition: defaultYPosition, value1: value1, value2: value2, scene: scene)
        
        // Set the walking frames for animation
        self.walkingAnimatedFrames = SpriteKitHelper.getTextureArrayFromAtlas(GameTextures.sharedInstance.airAtlas, texturesNamed: "steady", frameStart: 0, frameEnd: 15)
    ...
    override func setupTraitsWithScalar(_ scalar: Double) {
        ...
        self.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: self.size.width * 0.375, height: self.size.height / 4), center: CGPoint(x: -(self.size.width * 0.375), y: self.size.height / 22.0))
