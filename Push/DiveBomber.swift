...
class DiveBomber : Enemy {
    ...
    required init(scalar : Double, defaultYPosition: CGFloat, defaultXPosition: CGFloat, parent: SKNode, value1: Double, value2: Double, scene: GameScene) {
        super.init(scalar: scalar, imageName: "diver_flying_000", textureAtlas: GameTextures.sharedInstance.airAtlas, defaultYPosition: defaultYPosition, value1: value1, value2: value2, scene: scene)
        
        self.walkAction = SKAction.repeatForever(SKAction.animate(with: SpriteKitHelper.getTextureArrayFromAtlas(GameTextures.sharedInstance.airAtlas, texturesNamed: "diver_flying", frameStart: 0, frameEnd: 15), timePerFrame: 0.015, resize: true, restore: false))
        
        self.fightAction = SKAction.repeatForever(SKAction.animate(with: SpriteKitHelper.getTextureArrayFromAtlas(GameTextures.sharedInstance.airAtlas, texturesNamed: "diver_diving", frameStart: 0, frameEnd: 15), timePerFrame: 0.015, resize: true, restore: false))
    }
    ...
    override func setupTraitsWithScalar(_ scalar: Double) {
        ...
        self.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: self.size.width * 0.3, height: self.size.height * 0.2), center: CGPoint(x: self.size.width * -0.35, y: self.size.height * 0.0))
